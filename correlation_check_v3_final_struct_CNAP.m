...
%      Blocks.PhaseTime.WarmWater.Start = 600;
     ...
     lim = find([Blocks.Blocks(:).PainTime] > 300)
     star = find([Blocks.Blocks(:).PainTime] > 180)
     Standard_shape = quantile(Sample_matrix(star(1):lim(1)-1,:),0.5);
%      DStandard_shape = quantile(DSample_matrix(1:lim(1)-1,:),0.5);
     figure(iii)
     plot(Standard_shape)
%      
     for yyy=1: length(Blocks.Blocks)
     Blocks.Blocks(yyy).Error_vector(1:100) = Standard_shape-(Blocks.Normalized.Blocks(yyy).Signal(:)/max(Blocks.Normalized.Blocks(yyy).Signal(:)))';
     ...
     Blocks.Blocks(yyy).RMSE = sqrt(sum(SE)/100); 
...
%      end
 ...
 save(strcat('C:\OneDrive - Kaunas University of Technology\MAGISTRINIS\Data\Aurimod',num,'CNAP_Final_Struct.mat'), 'Blocks')
