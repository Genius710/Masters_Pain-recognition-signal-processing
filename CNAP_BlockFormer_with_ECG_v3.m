...
figure;plot(signal-mean(signal))
hold on
plot(signal_filtered)
...
peaks(find(peaks > length(signal))) = [];
...
Data(iii).Normalized.Signal = zeros(length(Data(iii).Blocks),100);
...
Data(iii).Normalized.Signal(yyy,:) = [ Data(iii).Normalized.Blocks(yyy).Signal ];
